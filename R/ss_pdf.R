...
ss_pdf <- function(..., citation_package = 'natbib') {
  ...
  base <- rticles:::pdf_document_format(
    "sa", highlight = NULL, citation_package = citation_package,
    template = system.file("rmarkdown", "templates", "sa", "resources",
      "template.tex", package = "sa4ss"),
    ...
  )
  ...
  base$post_processor <- function(metadata, utf8_input, output_file, clean, verbose) {
    ...
    h <- rticles:::get_list_element(metadata, c('output', 'rticles::sa', 'includes', 'in_header'))
  ...
  hook_chunk = function(x, options) {
    if (rticles:::output_asis(x, options)) return(x)
    paste0('```{=latex}\n\\begin{Schunk}\n', x, '\\end{Schunk}\n```')
  }
  ...
  base$knitr$knit_hooks <- rticles:::merge_list(base$knitr$knit_hooks, list(
    chunk   = hook_chunk,
    source  = hook_input,
    output  = hook_output,
    message = hook_output,
